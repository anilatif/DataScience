...
import pandas as pd
data = pd.read_csv('Zip_Zhvi_SingleFamilyResidence (1).csv',encoding='ISO-8859-1' )
...
#more stationarity == take the shift difference
unemploy_log_diffset = []
for i in range (0, len(log_unemploy_sets)):
 # print (log_unemploy_sets[i])
   unemploy_log_diffset.append(np.nan_to_num
                               (log_unemploy_sets[i]-log_unemploy_sets[i].shift()))
...
#for i,model in prophet_models.items():
#  for i in range(0,9):
#    model.plot(test_f[sorted_dict[:10][j][0]], uncertainty=True)
#    plt.ylabel('Median Housing Average (in dollars)')
#    plt.xlabel('Date')
#    plt.title(i)
#plt.show()
